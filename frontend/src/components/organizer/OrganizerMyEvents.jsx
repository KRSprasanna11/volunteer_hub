...
const OrganizerMyEvents = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8080/api/events/organizer/${organizerId}`)
      ...
      try {
        ...
        for (let event of events) {
          const res = await axios.get(
            `http://localhost:8080/api/applications/event/${event.id}`
          );
  ...
  const loadVolunteers = async (eventId) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:8080/api/applications/event/${eventId}`
      );
  ...
  const cancelEvent = async (eventId) => {
    ...
    try {
      await axios.put(`http://localhost:8080/api/events/${eventId}/cancel`);
